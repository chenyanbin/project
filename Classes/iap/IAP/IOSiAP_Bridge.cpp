...
// ［回调］支付结果返回
void IOSiAP_Bridge::onPaymentEvent(std::string &identifier, IOSiAPPaymentEvent event, int quantity,  std::string  &_receipt)
{
    ...
    else if (event == IOSIAP_PAYMENT_PURCHAED)
    {
        ...
        if (_paymentCallback)
        {
            _paymentCallback(true, identifier, quantity,_receipt);
        }
    }
    else if (event == IOSIAP_PAYMENT_FAILED)
    {
        ...
        if (_paymentCallback)
        {
            _paymentCallback(false, identifier, quantity,_receipt);
        }
    }
...
// 获取商品信息
void IOSiAP_Bridge::requestProducts(std::string &identifier, iapProductCallback callback)
{
...
// 付款请求
void IOSiAP_Bridge::requestPayment(int quantity, iapPaymentCallback callback)
{
    _quantity = quantity;
