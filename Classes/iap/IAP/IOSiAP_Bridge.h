...
typedef std::function<void(bool succeed, std::string &identifier, int quantity, std::string &_receipt)> iapPaymentCallback; // 支付结果回调
...
public:
    ...
    // ［回调］付款结果回调（恢复流程也走这里）
    virtual void onPaymentEvent(std::string &identifier, IOSiAPPaymentEvent event, int quantity, std::string  &_receipt);
    // ［回调］恢复购买完成回调
