//
//  RotateMenu.cpp
//  FightPass
//
//  Created by chenyanbin on 15/6/24.
//
//

#include "RotateMenu.h"
 #include "../../../protocol/UserCustomEvent.h"
#include <math.h>
#include "../../../protocol/UserCustomEvent.h"
#define PI acos(-1)
USING_NS_CC;
static int m_turn =0;
RotateMenu::RotateMenu()
:m_nowSelectedIndex(0)
,isturn(false)
{
//    m_turn = 0;
}
RotateMenu::~RotateMenu()
{
    _items.clear();
}
bool RotateMenu::init(){
    if (!Layer::init())
        return false;
    _angle = 0.0;
    this->ignoreAnchorPointForPosition(false);
    Size s = SDResource.size;
    this->setAnchorPoint(Point(0.5,0.5));
    this->setContentSize(s/3*2);
    
    auto listener = EventListenerTouchOneByOne::create();
//    listener->setSwallowTouches(true);
    listener->onTouchBegan = CC_CALLBACK_2(RotateMenu::onTouchBegan,this);
    listener->onTouchMoved = CC_CALLBACK_2(RotateMenu::onTouchMoved, this);
    listener->onTouchEnded = CC_CALLBACK_2(RotateMenu::onTouchEnded, this);
    getEventDispatcher()->addEventListenerWithSceneGraphPriority(listener, this);
    return true;
};

void RotateMenu::addMenuItem(cocos2d::MenuItem *item)
{
    addMenuItem(item, item->getZOrder(), item->getTag());
    return;
}
void RotateMenu::addMenuItem(cocos2d::MenuItem *item, int zOrder)
{
    addMenuItem(item, zOrder, item->getTag());

}
void RotateMenu::addMenuItem(cocos2d::MenuItem *item, int zOrder, int tag)
{
    item->setPosition(Point::ZERO);
    this->addChild(item,zOrder,tag);
    _items.pushBack(item);
    setUnitAngle(2 * PI / _items.size());
    reset();
    
    auto menuSize =this->getContentSize();
    auto disX = menuSize.width / 3.3;
    auto disY = menuSize.height / 4;
    for (int i = 0; i < _items.size(); i++)
    {
        float x = menuSize.width / 2 + disX*sin(i*_unitAngle + getAngle());
        float y = menuSize.height / 2 - disY*cos(i*_unitAngle + getAngle());
        
        _items.at(i)->setPosition(Point(x, y));
        _items.at(i)->setOpacity(192 + 63 * cos(i*_unitAngle + getAngle()));
        _items.at(i)->setScale(0.9 + 0.1*cos(i*_unitAngle + getAngle()));
        if(_items.at(i)->getScale() == 1)
        {
            this->m_nowSelectedIndex = i;
        }
    }   

    return;
}
void RotateMenu::updatePosition(){
    
    auto menuSize = getContentSize();
    auto disX = menuSize.width / 3.3;
    auto disY = menuSize.height / 4;
    for (int i = 0; i < _items.size(); i++){
        float x = menuSize.width / 2 + disX*sin(i*_unitAngle+getAngle());
        float y = menuSize.height / 2 - disY*cos(i*_unitAngle + getAngle());
        _items.at(i)->setPosition(Point(x, y));
        _items.at(i)->setZOrder(-(int)y);
        //Opacity  129~255
        _items.at(i)->setOpacity(192 + 63 * cos(i*_unitAngle + getAngle()));
        _items.at(i)->setScale(0.9 + 0.1*cos(i*_unitAngle + getAngle()));
    }
    return;
}
void RotateMenu::updatePositionWithAnimation()
{
    int nextSelectedIndex = 0;
    float scale = 0;
    auto menuSize = this->getContentSize();
    auto disX = menuSize.width / 3.3;
    auto disY = menuSize.height / 4;
    for (int i = 0; i < _items.size(); i++)
    {
        _items.at(i)->stopAllActions();
        float x = menuSize.width / 2 + disX*sin(i*_unitAngle + getAngle());
        float y = menuSize.height / 2 - disY*cos(i*_unitAngle + getAngle());
        auto moveTo = MoveTo::create(animationDuration, Point(x, y));
        _items.at(i)->runAction(moveTo);
        //Opacity  129~255
        auto fadeTo = FadeTo::create(animationDuration, (192 + 63 * cos(i*_unitAngle + getAngle())));
        _items.at(i)->runAction(fadeTo);
        //缩放比例  0.5~1
        scale = 0.9 + 0.1*cos(i*_unitAngle + getAngle());
      
        auto scaleTo = ScaleTo::create(animationDuration,scale);
        _items.at(i)->runAction(scaleTo);
        _items.at(i)->setZOrder(-(int)y);
        if(scale == 1)
        {
            nextSelectedIndex = i;
        }
    }
    DelayTime* delay = DelayTime::create(animationDuration);
    CallFunc* callBack = CallFunc::create(CC_CALLBACK_0(RotateMenu::onMoveEnded, this,nextSelectedIndex));
    this->stopAllActions();
    this->runAction(Sequence::create(delay,callBack, NULL));
    return;
}
void RotateMenu::onMoveEnded(int nextSelected)
{
    log("nextSelected==%d",nextSelected);
    if(isturn== true)
    {
        broadMsg(CUSTOM_LINEUP_QIEHUANWUJIANG,__Integer::create(nextSelected));
    }
}
int RotateMenu::getSelected()
{
    
    return m_nowSelectedIndex;
}
void RotateMenu::reset(){
    _angle = 0;
    m_nowSelectedIndex = 0;
}
void RotateMenu::setAngle(float angle){
    this->_angle = angle;
}
float RotateMenu::getAngle(){
    return _angle;
}
void RotateMenu::setUnitAngle(float angle){
    _unitAngle = angle;
}
float RotateMenu::getUnitAngle(){
    return _unitAngle;
}

float RotateMenu::disToAngle(float dis){
    float width = this->getContentSize().width / 2;
    return dis / width*getUnitAngle();
}

MenuItem * RotateMenu::getCurrentItem(){
    if (_items.size() == 0)
        return nullptr;
    //这里实际加上了0.1getAngle(),用来防止精度丢失
    int  index = (int)((2 * PI - getAngle()) / getUnitAngle()+0.1*getUnitAngle());
    index %= _items.size();
    return _items.at(index);
}


bool RotateMenu::onTouchBegan(Touch* touch, Event* event){
    //先停止所有可能存在的动作
    isturn = false;
    touchBeginPos = touch->getLocation();
    for (int i = 0; i < _items.size(); i++)
        _items.at(i)->stopAllActions();
    auto position = this->convertToNodeSpace(touch->getLocation());
    auto size = this->getContentSize();
    auto rect = Rect(0, 0, size.width, size.height);
    if (rect.containsPoint(position)){
        return true;
    }
    return false;
}
void RotateMenu::onTouchEnded(Touch* touch, Event* event)
{
    log("---------------------%f",touch->getLocation().x - touchBeginPos.x);
    
    auto xDelta = touch->getLocation().x - touch->getStartLocation().x;
    rectify(xDelta>0);
       updatePositionWithAnimation();
    return;
}
void RotateMenu::onTouchMoved(Touch* touch, Event* event){
    isturn = true;
    auto angle = disToAngle(touch->getDelta().x);
    setAngle(getAngle() + angle);
    updatePosition();
    return;
}

void RotateMenu::rectify(bool forward){
    auto angle = getAngle();
    while (angle<0)
        angle += PI * 2;
    while (angle>PI * 2)
        angle -= PI * 2;
    if(forward>0)
        angle = ((int)((angle + getUnitAngle() / 3*2) / getUnitAngle()))*getUnitAngle();
    else
        angle = ((int)((angle + getUnitAngle() / 3 ) / getUnitAngle()))*getUnitAngle();
    setAngle(angle);
}

void  RotateMenu::onMsgProcess(EventCustom* msg)
{
    m_turn = 0;
}
void RotateMenu::addEventListener()
{
//    addUserMsg(CUSTOM_LINEUP_QIEHUANWUJIANG, CC_CALLBACK_1(RotateMenu::onMsgProcess, this));
}
void RotateMenu::removeEventListener()
{
    BaseLayer::removeEventListener();
}
