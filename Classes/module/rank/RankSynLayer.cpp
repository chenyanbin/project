...
#include "../../message/Decoding/ranking/RankingMultipleMsg.hpp"
...
RankSynLayer* RankSynLayer::create()
{
    RankSynLayer* layer = new RankSynLayer();
    if(layer && layer->init())
    {
        layer->autorelease();
        return layer;
...
void RankSynLayer::onEnter()
{
    BaseLayer::onEnter();
    
}
void RankSynLayer::onExit()
{
    BaseLayer::onExit();
}
void RankSynLayer::onExitTransitionDidStart()
{
    BaseLayer::onExitTransitionDidStart();
    removeEventListener();

}
void RankSynLayer::onEnterTransitionDidFinish()
{
    BaseLayer::onEnterTransitionDidFinish();
    requestSynRankingMsg();
    
}



void RankSynLayer::setPersonBg()
{
    Sprite* personBg = Sprite::create(IMG_RANK_PERSON);
    personBg->setAnchorPoint(Point::ZERO);
    this->addChild(personBg,2,TAG_PERSONBG);
    
    Button* button = Button::create(IMG_PUBLIC_BUTTON_FOUR_RED, IMG_PUBLIC_BUTTON_FOUR_RED, CC_CALLBACK_1(RankSynLayer::onPriceClick, this));
    button->setAnchorPoint(Point::ZERO);
    button->setPosition(100,125);
    personBg->addChild(button,1);
...
void RankSynLayer::onPriceClick(Ref* pSender)
{
    RankPriceLayer::show();
}
void RankSynLayer::setperdetail()
{
...
void RankSynLayer::setTableView()
{
...
Sprite* RankSynLayer::singleList(size_t idx)
{
...
//    //得分
    ...
    acativeLabel->setAnchorPoint(Point::ZERO);
    acativeLabel->setPosition(150,80);
    ...
    Label* levelabel = PublicShowUI::getLabel(levelstr.c_str(),PublicShowUI::getColor3b(COLOR_GREEN_WORD),18,false,false);
    levelabel->setAnchorPoint(Point::ZERO);
    levelabel->setPosition(fightlabel->getPositionX() + fightlabel->getContentSize().width+20,30);
    cellbg->addChild(levelabel,3);
    
    Button* btn = Button::create(IMG_PUBLIC_BUTTON_TWO_ORANGE, IMG_PUBLIC_BUTTON_TWO_ORANGE, CC_CALLBACK_1(RankSynLayer::onfriendclick, this));
...
void RankSynLayer::onfriendclick(Ref* pSender)
{
...
TableViewCell* RankSynLayer::tableCellAtIndex(TableView *table, ssize_t idx)
{
...
ssize_t RankSynLayer::numberOfCellsInTableView(TableView *table)
{
    return RankingDataManager::getInstance()->rankingList->size();
}
void RankSynLayer::tableCellTouched(TableView* table, TableViewCell* cell)
{
    
}
Size RankSynLayer::tableCellSizeForIndex(TableView *table,ssize_t idx)
{
...
//-----------------------------------------------------访问服务器-------------------------------------------------//
void RankSynLayer::requestSynRankingMsg()//请求列表信息
{
    Dialog::showServerLoading();
    RankingMultipleMsg* msg = new RankingMultipleMsg();
    NetworkDataProcessing::getInstance()->sendMessage(msg);
    msg = NULL;
}
void RankSynLayer::onSynRankingProcess(EventCustom* msg)
{
    Dialog::hideServerLoading();
    MessageReceive* message = (MessageReceive*)msg->getUserData();
    RankingDataManager::getInstance()->clearRankingList();
    if(message->messageID == SCENSE_CLIENT_LEGEND_LegendRankFinalPageResp)
    {
...
void RankSynLayer::addEventListener()
{
    this->addUserMsg(PublicShowUI::numberToString(SCENSE_CLIENT_LEGEND_LegendRankFinalPageResp).c_str(), CC_CALLBACK_1(RankSynLayer::onSynRankingProcess, this));
}
void RankSynLayer::removeEventListener()
{
